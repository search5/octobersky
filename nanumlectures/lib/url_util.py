...
class UserConverter(BaseConverter):
    def to_python(self, value):
        record = db_session.query(User.username, User.name, User.email, User.phone, UserSocialAuth.provider,
                                  User.last_login_date, User.usertype, Library.id, Library.library_name,
                                  User).join(UserSocialAuth, User.id == UserSocialAuth.user_id).outerjoin(
            Library).filter(User.username == value).first()
